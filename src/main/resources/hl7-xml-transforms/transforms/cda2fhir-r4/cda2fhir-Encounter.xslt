    ...
    <Encounter>
      <xsl:choose>
        <xsl:when test="$gvCurrentIg = 'eICR'">
          ...
          <text>
            <xsl:choose>
              <xsl:when test="count(//cda:section[cda:templateId/@root='2.16.840.1.113883.10.20.22.2.22.1']/cda:entry) = count(//cda:section[cda:templateId/@root='2.16.840.1.113883.10.20.22.2.22.1']/cda:entry[@typeCode = 'DRIV'])">
      ...
      <!-- RG: The above comment and the eICR specific code below is based on a false assumption. Encounter.class is not a code/translation thing, it is a codeSystem thing in this case. 
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:call-template name="author-reference">
            <xsl:with-param name="pElementName">participant</xsl:with-param>
          </xsl:call-template>
        </xsl:otherwise>
      </xsl:choose>
